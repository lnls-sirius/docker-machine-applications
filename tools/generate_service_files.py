...
def generate_service_2_ioc_dict(appdir_path):  # noqa: C901
...
def generate_service_2_ioc_table():  # noqa: C901
    ...
    data = generate_service_2_ioc_dict(appdir_path)
...
def generate_stack_2_beagle_table():  # noqa: C901
    """Generate docker service satcks -> Beaglebone IP table."""
    appdir_path = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), '..', 'apps')
    data = generate_service_2_ioc_dict(appdir_path)
    text, _ = util.read_text_data(beaglebone_ip_list())
    bbbname2ip = {item[0]: item[1] for item in text}

    fname = 'services2bbb.txt'
    hmsg = '# This is a txt file that was automatically generated by\n'
    hmsg += '# the generate_service_files.py script, available at the\n'
    hmsg += '# docker-machine-applications repository.\n'
    hmsg += '#\n'
    hmsg += '# The blocks below define the relation:\n'
    ...
    hmsg += '#       %PSNAME2\n'
    hmsg += '# ...\n'
    hmsg += '# where STACK is the name of the high level docker\n'
    hmsg += '# service stack, SERVICE is the docker service, \n'
    hmsg += '# BBBIP is the beaglebone IP and PSNAME1 is the\n'
    hmsg += '# firt power supply name, and so on....\n\n'

    with open(fname, 'w') as file:
        file.write(hmsg)
        for stack in ServiceConfig.STACKS:
            if 'ps' not in stack or stack in ('si-ps-fastcorrs', 'li-ps'):
                continue
            file.write('!' + stack + '\n')
            services = ServiceConfig.STACKS[stack]
            for service in services:
                container = 'facs-' + stack + '_' + service
                iocs = data[container]
                file.write(' '*4 + '@' + container + '\n')
                for ioc in iocs:
                    ioc_type, *arg = ioc.split()
                    if ioc_type == 'as-ps':
                        bbb_hostname = arg[0]
                        bbb_ip = bbbname2ip[bbb_hostname]
                        psnames = PSSearch.conv_bbbname_2_psnames(bbb_hostname)
                        file.write(' '*8 + '$' + bbb_ip + '\n')
                        for psname, _ in psnames:
                            file.write(' '*12 + '%' + psname + '\n')


if __name__ == "__main__":
    generate_service_files()
    generate_service_2_ioc_table()
    generate_stack_2_beagle_table()
